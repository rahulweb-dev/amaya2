      ...
      <div className="bg-navy pt-28 pb-14 px-6 lg:px-16">
        <div className="max-w-[1200px] mx-auto">
          ...
          <p className="font-sans font-light text-stone/65 text-[15px] max-w-lg leading-relaxed">
            Five configurations. Every one designed for accessibility, light, ventilation, privacy,
            and quiet comfort. Floor plan illustrations are indicative and subject to final
            regulatory approval.
          </p>
        </div>
      </div>
              ...
              {[
                {
                  ...
                  body: 'Wider doorways, lever handles, slip-resistant floors, ramp access, and considered lighting.',
                },
                {
                  title: 'Light and air',
                  body: 'Cross ventilation, generous balconies, and large windows in every residence.',
                },
                {
                  title: 'Privacy by layout',
                  body: 'Apartments planned to keep daily life private, with shared community spaces nearby.',
                },
                {
                  title: 'Maintenance, handled',
                  body: 'Long-term maintenance and management by the Vera Vita team. Every detail, looked after.',
                },
              ].map((p) => (
                <div
                  key={p.title}
                ...
                >
                  <div className="w-7 h-0.5 bg-sage mb-4" />
                  <h3 className="font-serif text-base font-normal text-charcoal mb-2 leading-snug">
                    {p.title}
                  </h3>
                  <p className="font-sans font-light text-[12.5px] text-text-mid leading-relaxed">
                    {p.body}
                  </p>
                </div>
              ))}
              ...
              <Reveal key={res.title} delay={i * 0.08}>
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <h3 className="font-serif text-xl font-normal text-charcoal mb-1">{res.title}</h3>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        onClick={() => openModal('floorplan', `residence:${res.slug}`)}
          ...
          <Reveal>
            <div className="bg-white border border-stone rounded p-6 lg:p-8 overflow-x-auto">
              ...
              <p className="font-sans font-light text-text-mid text-sm mb-5">
                All areas and specifications are indicative and subject to final approval.
              </p>
              <table className="w-full font-sans text-sm border-collapse min-w-[560px]">
                <thead>
                  <tr className="border-b-2 border-stone">
                    ...
                    {['1 BHK', '2 BHK', '2.5 BHK', '3 BHK', '3.5 BHK'].map((t, i) => (
                      <th
                        key={t}
                  ...
                  {[
                    {
                      ...
                      values: ['680 sq ft', '950 sq ft', '1,150 sq ft', '1,400 sq ft', '1,700 sq ft'],
                    },
                    { label: 'Balconies', values: ['1', '2', '2', '2', '3'] },
                    { label: 'Study / flex room', values: ['-', '-', 'Yes', 'Yes', 'Yes'] },
                    { label: 'Guest bedroom', values: ['-', '-', '-', 'Yes', 'Yes'] },
                  ].map((row) => (
                    <tr key={row.label} className="border-b border-mist">
                      <td className="py-2.5 px-3 text-text-mid font-light">{row.label}</td>
                      {row.values.map((v, i) => (
                        <td
                          key={i}
